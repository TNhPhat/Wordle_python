...
from environment.setttings import COLORS,FONT_PATH,BORDER_RADIUS
import pygame
...
class keyboard_key(Drawable):
    def __init__(self,character,width,height,scale = [1,1]):
        ...
        self.font = pygame.font.Font(FONT_PATH, 3*max(width,height) //(4*len(character)))
        

    def update_layout(self, scale, window_width, window_height):
        self.set_scale(scale)
        self.font = pygame.font.Font(FONT_PATH,round(3*max(self.rect.width*self.scale[0],self.rect.height*self.scale[1]) / (4*len(self.character))))
        return 
